...
export const createHierarchy = (options) =>{
    ...
    setSelIndiaData, setSelStateData, setSwitchDisplay, setSelDistrictsData)
  {
    ...
    if (level === 1)
    {
     ...
     setSelIndiaData(solr_body_3.response.docs);
      ...
      let solr_url_4;
      if (levelThree){
        solr_url_4 =  await fetch(`http://nutritionindiainfo.communitygis.net:8983/solr/nutritionv16/select?fl=area_id%2Carea_code%2Carea_name%2Carea_level%2Cdata_value%2Cdata_value_num&fq=area_parent_id%3A${parentArea}&fq=indicator_id%3A${indicator}&fq=subgroup_id%3A6&fq=timeperiod_id%3A${timeperiod}&rows=1000&omitHeader=true&q=*%3A*`);
    ...
    const solr_switchurl= await fetch(`http://nutritionindiainfo.communitygis.net:8983/solr/nutritionv16/select?fl=indicator_id%2Cindicator_name%2Ctimeperiod_id%2Ctimeperiod%2Cunit_id%2Cunit_name%2Cdata_value%2Cdata_value_num%2Carea_id%2Carea_code%2Carea_name%2Carea_level&fq=area_level%3A3&fq=indicator_id%3A${indicator}&fq=subgroup_id%3A6&fq=timeperiod_id%3A${timeperiod}&q=*%3A*&rows=10000&omitHeader=true`);
    ...
    setSelIndicator, setUnit, setGraphTitle, setGraphUnit, selArea, parentArea, level, isLevelThree, setIndicatorBar, setIndicatorTrend, setSelIndiaData, setSelStateData, setSwitchDisplay, setSelDistrictsData, setTimeperiodDropdownOpt, setSelTimeperiod, setGraphTimeperiod, setIndicatorSense,queryIndicator)
  {
    ...
    const solr_url_6 = await fetch(`http://nutritionindiainfo.communitygis.net:8983/solr/nutritionv16/select?fl=value:indicator_id%2Ctitle:indicator_name%2Cindi_sense&fq=category_id%3A${selCategory}&fq=lifecycle_id%3A${selLifeycle}%20OR%20lifecycle_id%3A7&q=*%3A*&rows=100&sort=indicator_id%20asc&group=true&group.field=indicator_id&group.limit=1&group.main=true&omitHeader=true`);
    ...
    const solr_url_8 = await fetch(`http://nutritionindiainfo.communitygis.net:8983/solr/nutritionv16/select?fl=unit_id%2Cunit_name%2Cindicator_id&fq=indicator_id%3A${indiVal}&fq=subgroup_id%3A6&group.field=unit_id&group.main=true&group=true&omitHeader=true&q=*%3A*`);
