...
export const createHierarchy = (options) =>{
    ...
    setSelIndiaData, setSelStateData, setSwitchDisplay, setSelDistrictsData)
  {
   ...
   // const url_2 = await fetch(`http://localhost:8000/api/getIndicatorBar/${indicator}/${timeperiod}/${area}`);
    ...
    //  const url_3 = await fetch(`http://13.234.11.176/api/indiaMap/${indicator}/6/${timeperiod}/2`);
     const solr_url_3 = await fetch(`http://nutritionindia.communitygis.net:8983/solr/nutritionv12/select?fl=area_id%2Carea_code%2Carea_name%2Carea_level%2Cdata_value%2Cdata_value_num&fq=area_level%3A2&fq=indicator_id%3A${indicator}&fq=subgroup_id%3A6&fq=timeperiod_id%3A${timeperiod}&rows=100&omitHeader=true&q=*%3A*`);
    ...
    setSelIndicator, setUnit, setGraphTitle, setGraphUnit, selArea, parentArea, level, isLevelThree, setIndicatorBar, setIndicatorTrend, setSelIndiaData, setSelStateData, setSwitchDisplay, setSelDistrictsData, setTimeperiodDropdownOpt, setSelTimeperiod, setGraphTimeperiod, setIndicatorSense)
  {
    ...
    const solr_url_8 = await fetch(`http://nutritionindia.communitygis.net:8983/solr/nutritionv12/select?fl=unit_id%2Cunit_name%2Cindicator_id&fq=indicator_id%3A${indiVal}&fq=subgroup_id%3A6&group.field=unit_id&group.main=true&group=true&omitHeader=true&q=*%3A*`);
   ...
   // const solr_url = await fetch(`http://nutritionindia.communitygis.net:8983/solr/nutritionv12/select?fl=title:timeperiod%2Cvalue:timeperiod_id&sort=timeperiod_id%20desc&q=indicator_id%3A${indiVal}%20AND%20subgroup_id%3A6%20AND%20area_id%3A${selArea}`);
        // const body_2 = await url_2.json();
    let solr_url;
    // if(isLevelThree){
      // url = await fetch(`http://13.234.11.176/api/timeperiod/${val}/6/${parentArea}`);
      solr_url = await fetch(`http://nutritionindia.communitygis.net:8983/solr/nutritionv12/select?fl=title:timeperiod%2Cvalue:timeperiod_id&sort=timeperiod_id%20desc&fq=lifecycle_id%3A${selLifeycle}%20OR%20lifecycle_id%3A7&fq=category_id%3A${selCategory}&fq=indicator_id%3A${indiVal}&fq=subgroup_id%3A6&fq=area_id%3A${selArea}&q=*%3A*&group=true&group.field=timeperiod_id&group.limit=1&group.main=true&omitHeader=true`);

    // }
    // else{
    //       // url = await fetch(`http://13.234.11.176/api/timeperiod/${val}/6/${selArea}`);
    //       solr_url = await fetch(`http://nutritionindia.communitygis.net:8983/solr/nutritionv12/select?fl=title:timeperiod%2Cvalue:timeperiod_id&sort=timeperiod_id%20desc&q=lifecycle_id%3A${selLifeycle}%20AND%20category_id%3A${selCategory}%20AND%20indicator_id%3A${indiVal}%20AND%20subgroup_id%3A6%20AND%20area_parent_id%3A${selArea}&group=true&group.field=timeperiod_id&group.limit=1&group.main=true&omitHeader=true`);
    // }
